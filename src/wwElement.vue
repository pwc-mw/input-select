  ...
  >
    ...
    <template #singlelabel="{ value }">
      <div
        ...
        :style="value.style || defaultOptionStyle"
      >
        <wwLayoutItemContext
...
export default {
  ...
  setup(props) {
    const { value: currentSelection, setValue: setCurrentSelection } =
      wwLib.wwVariable.useComponentVariable({
        uid: props.uid,
        name: "value",
        type: "string",
        defaultValue: computed(() =>
          props.content.initialValue ? props.content.initialValue : ""
        ),
      });
  ...
  computed: {
      ...
      get() {
        if (this.content.allowCreation) {
          ...
          if (
            ...
            !this.options.some(
              (option) => option.value === this.currentSelection
            )
          ) {
    ...
    valueLabel() {
      const _option = this.options.find(
  ...
  watch: {
    ...
    isEditing() {
      this.handleOpening(
        !this.isEditing
          ? false
          : this.wwEditorState.sidepanelContent.openInEditor
      );
    },
    /* wwEditor:end */
    textStyle() {
  ...
  methods: {
    ...
    async init() {
      ...
      this.options = [];
      const initialOptions = Array.isArray(this.content.options)
        ? [...this.content.options]
        : [];
      this.options.push(
        ...initialOptions.map((option) => this.formatOption(option))
      );
      ...
      if (
        ...
        !this.options.some(
          (option) =>
            option === this.content.initialValue ||
            option.value === this.content.initialValue
        )
      ) {
    ...
    formatOption(option) {
      ...
      if (this.content.layoutType === "free")
        return {
          label: wwLib.wwLang.getText(
            wwLib.resolveObjectPropertyPath(option, labelField)
          ),
      ...
      return typeof option === "object"
        ? {
            ...
            style: {
              backgroundColor:
                wwLib.resolveObjectPropertyPath(option, "bgColor") ||
                "#FFFFFF00",
              color:
